...
export function PageWithHeader({
  ...
  const content = (
    <div
      className={cn(
        fluid ? "px-0 container mx-auto" : "max-w-5xl mx-auto px-4"
      )}
    >
  ...
  return (
    <div>
      {content}
      <Tabs defaultValue={defaultTab ?? tabTriggers[0].value} className="mb-8">
        <TabsList className="w-full justify-start gap-6 h-auto p-0 rounded-none border-b border-sline-base-border-alpha">
          <div
            className={cn(
              fluid ? "container mx-auto" : "max-w-5xl mx-auto",
              "flex mx-auto px-4 w-full justify-start gap-6 h-auto p-0"
            )}
          >
            {tabTriggers.map((tab) => (
              <TabsTrigger
