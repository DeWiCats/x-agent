...
Output the tweet text only, no explanations.`;
  ...
  while (attempts < 3) {
    ...
    let actualResponse = veniceResponse;
    let trimmedResponse = actualResponse.trim().replace(/^["']|["']$/g, "");

    // Only use deepseek if multi_model is true
    if (agent.multi_model) {
...
Tweet to improve:`,
      ...
      actualResponse = deepseekResponse.split("</think>\n\n")[1];
